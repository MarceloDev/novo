...
//FUNÇÃO PARA RETIRAR CARACTRES DA STRING
function trim(str) {
...
function myCallback(time, tipo, titulo, conteudo) {
    // tipes: success warning info error
    if (time == '') {
        time = 3000;
    } else if (time == '0') {
        time = 100000;
    }
    var numero = 5000 + Math.floor(Math.random() * 5001);

    $('#mensagemAlert').prepend(
            '<div class="toast toast-' + tipo + '" style="display:none;" id="exibe_' + numero + '">'
            + '<button class="toast-close-button" id="' + numero + '">'
            + '×'
            + '</button>'
    ...
    $("#exibe_" + numero).fadeIn("slow");

    $("#" + numero).click(function() {
        $("#exibe_" + numero).fadeOut("slow", function() {
            $(this).remove();
        });
    });
    ...
    $("#exibe_" + numero).mousemove(function() {
        clearTimeout(timer);
...
function myModal(id, titulo, content, buttom, form) {

    $('.modal').attr('id', id).html(
            '<div class="modal-dialog"  style="width: 60%;">'
            + '<form name="' + form + '" method="post" >'
            + '<div class="modal-content">'
            + '<div class="modal-header">'
            + '<button type="button" class="close j_close" data-dismiss="modal" aria-hidden="true">&times;</button>'
            ...
            + '<button type="button" class="btn btn-default j_close" data-dismiss="modal">Cancelar</button>'
            + buttom
            + '</div>'
            + '</div>'
            + '</form>'
